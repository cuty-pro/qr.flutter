...
//        paint);
    ...
    for (var x = 0; x < _qr!.moduleCount; x++) {
      for (var y = 0; y < _qr!.moduleCount; y++) {
        ...
        final squareRect = Rect.fromLTWH(
          left,
          top,
          paintMetrics.pixelSize + (dataModuleStyle.pixelHTweak ?? pixelHTweak),
          paintMetrics.pixelSize + (dataModuleStyle.pixelVTweak ?? pixelVTweak),
        );
  ...
  void _drawFinderPatternItem(
    ...
    if (eyeStyle.eyeShape == QrEyeShape.square) {
      ...
      double paintRadius;
      double paintInnerRadius;
      double paintDotRadius;
      if (eyeStyle.eyeShape == QrEyeShape.radius) {
        paintRadius = eyeStyle.radius;
        paintInnerRadius = paintDotRadius = paintRadius * (dotSize / radius);
      } else {
        paintRadius = radius;
        paintInnerRadius = innerRadius;
        paintDotRadius = dotSize;
      }

      final roundedOuterStrokeRect =
          RRect.fromRectAndRadius(outerRect, Radius.circular(paintRadius));
      canvas.drawRRect(roundedOuterStrokeRect, outerPaint);

      final roundedInnerStrokeRect =
          RRect.fromRectAndRadius(outerRect, Radius.circular(paintInnerRadius));
      canvas.drawRRect(roundedInnerStrokeRect, innerPaint);

      final roundedDotStrokeRect =
          RRect.fromRectAndRadius(dotRect, Radius.circular(paintDotRadius));
